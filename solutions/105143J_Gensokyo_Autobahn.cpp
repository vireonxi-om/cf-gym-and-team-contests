// Problem: Gensokyo Autobahn
// URL: https://codeforces.com/gym/105143/problem/J
// Rating: unrated
...
namespace Poly {
    ...
    */
    ...
    void fft(complex y[],int len,int type) {
        ...
        if(type == -1) for(int i = 0; i < len; i++) y[i].r /= len, y[i].i /= len;
    }
#endif
    ...
    void init() {
        for (int h = 2; h <= MAXN; h <<= 1) {
            ...
            W[h][0] = W[h][1] = 1;
            for (int d = 1; d < h / 2; d++) {
                W[h + d][0] = (LL)x * W[h + d - 1][0] % mod;
                W[h + d][1]=(LL)y * W[h + d - 1][1] % mod;
            }
        }
    }
    ...
    void fft(int y[], int len, int type) {
        ...
        if(type == -1) {
            for (int i = 0, x = pow_mod(len, mod - 2); i < len; i++)
                y[i] = (LL)y[i] * x % mod;
        }
    }
#endif
    ...
    struct poly {
        ...
        poly operator - () const {
            poly res(length);
            repn(i, 0, length) res[i] = sub(0, a[i]);
            return res;
        }
        poly operator * (const T &p) const {
            poly res(length);
            repn(i, 0, length) res[i] = mul(a[i], p);
            return res;
        }
        poly operator + (const T &p) const {
            poly res(length);
            repn(i, 0, length) res[i] = add(a[i], p);
            return res;
        }
        poly operator - (const T &p) const {
            poly res(length);
            repn(i, 0, length) res[i] = sub(a[i], p);
            return res;
        }
        // brute force for small poly
        void mul(T *a, T *b, T *c, int lengtha, int lengthb, int lengthret, int n) const {
            repn(i, 0, n) c[i] = 0;
            repn(i, 0, lengtha) repn(j, 0, std::min(lengthb, n - i))
        ...
        void conv(T *a, T *b, T *c, int lengtha, int lengthb, int lengthret, int n) const {
            ...
            fft(a1, lengthret, 1);
            fft(a2, lengthret, 1);
            for (int i = 0; i < lengthret; i++) a1[i] = a1[i] * a2[i];
            fft(a1, lengthret, -1);
            for (int i = 0; i <= n; i++) c[i] = (a1[i].r + 0.5);
            delete [] a1;
            delete [] a2;
        }
        poly operator * (const poly &p) const {
            if (length == -1 || p.length == -1) return poly(-1);
            int n = length + p.length, lengthret = 1;
        ...
        void conv(T *a, T *b, T *c, int lengtha, int lengthb, int lengthret, int n) const {
            ...
            fft(ra, lengthret, -1);
            repn(i, 0, n) {
                c[i]=(LL)(ka[i].r + 0.5) % mod * (LL)s * s % mod;
                c[i] = (c[i] + (LL)(kb[i].r + 0.5) % mod * (LL)s + (LL)(ra[i].r + 0.5)) % mod;
            }
            delete [] ka;
            delete [] ra;
            delete [] kb;
...
#ifdef FWT
        poly operator ^ (const poly &p) const {
            int n = std::max(length, p.length), lengthret = 1;
            for (; lengthret <= n; lengthret <<= 1);
            int *a1 = new int [lengthret];
            int *a2 = new int [lengthret];
            for (int i = 0; i < lengthret; i++) {
        ...
        poly operator * (const poly &p) const {
            ...
            poly res(n);
            for (int i = 0; i<= n; i++) res[i] = a3[set_size[i]][i];
            for (int i = 0; i < BASE; i++) {
                delete [] a1[i];
                delete [] a2[i];
                delete [] a3[i];
        ...
        poly operator % (const poly &p) const {
            if (length < p.length) return (*this);
...
int main() {
    ...
    repn(i, 0, n) if (dis[i] != -1) h[dis[i]].pb(i);
    repn(i, 0, n) {
        for (auto x: h[i]) {
            for (auto y: g[x]) if (dis[y] == dis[x] + 1) dp[y] = (dp[y] + dp[x]) % mod;
        }
    }
    work(1, 1, k);
    fac[0] = 1;
    ...
    repn(i, 0, dis[n]) {
        if (k < i) break;
        LL v = f[1][i];
        v = v * pow_mod(m, k - i) % mod;
        v = v * C(dis[n], i) % mod;
        v = v * fac[i] % mod;
        ans = (ans + v) % mod;
    }
    ans = ans * dp[n] % mod;
    ans = ans * pow_mod(pow_mod(m, k), mod - 2) % mod;
    cout << ans << "\n";
    return 0;
}